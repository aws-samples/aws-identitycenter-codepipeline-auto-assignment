...
# Temporary stream handler to print logs on screen for local test
...
def get_account_id_by_name(account_name):
    ...
    try:
        ...
        log_and_append_error(f"Error getting account ID for name {account_name}: {error}")
        return None
    ...
    try:
        ...
        for page in paginator.paginate(ParentId=parent_id):
            for ou in page['OrganizationalUnits']:
                if ou['Name'] == ou_name:
                    return ou['Id']
                nested_ou_id = find_ou_id(orgs_client, ou['Id'], ou_name)
                if nested_ou_id:
                    return nested_ou_id
        return None
...
def get_accounts_in_ou(ou_name):
    """Get list of account IDs in an organizational unit by OU name"""
    account_ids = []
    try:
        ...
        log_and_append_error(f"Error getting accounts for OU {ou_name}: {error}")
        return account_ids


# def get_accounts_in_ou(orgs_client, ou_name):
#     """Get list of account IDs in an organizational unit by OU name"""
#     account_ids = []
#     try:
#         # First, get the organization root ID
#         root_response = orgs_client.list_roots()
#         root_id = root_response['Roots'][0]['Id']
        
#         # Find the OU ID from the OU name
#         ou_id = None
#         paginator = orgs_client.get_paginator('list_organizational_units_for_parent')
...
#         return account_ids
#     except ClientError as error:
#         logger.error(f"Error getting accounts for OU {ou_name}: {error}")
#         return account_ids
...
def drift_detect_update(all_assignments, global_file_contents,
        ...
        try:
            ...
            for global_mapping in global_file_contents:
                for each_perm_set_name in global_mapping['PermissionSetName']:
                    global_group_id = get_valid_group_id(global_mapping['GlobalGroupName'])
                    if not global_group_id:
                        continue
                    permission_set_arn = get_permission_set_arn(each_perm_set_name, current_aws_permission_sets)
            ...
            for target_mapping in target_file_contents:
                ...
                if account_id in target_accounts:
                    for each_perm_set_name in target_mapping['PermissionSetName']:
                        ...
                        if not permission_set_arn:
                            continue
                        target_group_id = get_valid_group_id(target_mapping['TargetGroupName'])
    ...
    for item in remove_list:
        try:
            ...
            logger.warning(f'Remove list item {item} not in check list. Skipping...')
    ...
    if len(check_list) == 0:
        ...
        for delta_assignment in check_list:
            try:
                ...
                group_name=get_group_name_from_id(delta_assignment['PrincipalId'])
                if not group_name:
                    group_name=delta_assignment['PrincipalId']
                perm_set_name=get_perm_set_name_from_arn(delta_assignment['PermissionSetArn'])
...
def validate_mapping_file_structure(permission_set_file, group_type):
    ...
    for idx, permission_set in enumerate(permission_set_file):
        if not isinstance(permission_set, dict):
            raise TypeError(f"Each item in the mapping file must be a dictionary. Item at index {idx} is not a dictionary.")
        
        if group_key not in permission_set:
            raise ValueError(f"Missing required key: '{group_key}' in mapping file at index {idx}")
        
        if not isinstance(permission_set[group_key], str):
            raise TypeError(f"Key '{group_key}' is not of expected type str at index {idx}")
        ...
        # Validate 'TargetAccountid' based on the group type   
        if "TargetAccountid" not in permission_set:
            raise ValueError(f"Missing required key: 'TargetAccountid' in mapping at index {idx}")
        
        if not isinstance(permission_set["TargetAccountid"], target_accountid_type):
            raise TypeError(f"'TargetAccountid' must be of type {target_accountid_type.__name__} in mapping file at index {idx}")
        ...
        if not all(isinstance(item, str) for item in permission_set["PermissionSetName"]):
            raise ValueError(f"All items in 'PermissionSetName' must be strings in permission sets at index {idx}")
        
        # Additional check for 'TargetAccountid' to ensure it's a list of strings
        if group_type == "target" and not all(isinstance(item, str) for item in permission_set["TargetAccountid"]):
            raise ValueError(f"All items in the list 'TargetAccountid' must be strings in account IDs at index {idx}")
...
def global_group_array_mapping(acct_list, global_file_contents,
    ...
    for acct in acct_list:
        if acct['Status'] in ["SUSPENDED", "PENDING_CLOSURE"]:
            logger.info(f"Account ID: {acct['Id']} is in {acct['Status']} status and will be skipped.")
            ...
            if account['Status'] not in ["SUSPENDED", "PENDING_CLOSURE"]:
                for index, mapping in enumerate(global_file_contents):
                    if mapping['TargetAccountid'].upper() == "GLOBAL":
                        try:
                            logger.debug("Processing mapping: %s for account: %s", mapping, account)
...
def target_group_array_mapping(acct_list, target_file_contents,
    ...
    target_accounts = [
        acct for acct in acct_list 
        if (acct['Id'] != management_account_id and 
            acct['Status'] not in ["SUSPENDED", "PENDING_CLOSURE"])
            ]


    group_type = 'target'

    # Validate the structure of the target file contents
    try:
    ...
    if target_file_contents:
        try:
            for mapping in target_file_contents:
                ...
                for target in mapping['TargetAccountid']:
                    if target.startswith('name:'):
                        ...
                        if ou_account_ids:
                            ...
                            logger.warning(f"No accounts found in OU: {ou_name}")
                    else:
                        # Handle direct account ID
                        for acct in target_accounts:
                ...
                mapping_target_accounts = list(dict.fromkeys(mapping_target_accounts))
                ...
                for each_perm_set_name in mapping['PermissionSetName']:
                    ...
                    for target_account_id in mapping_target_accounts:
                        ...
                        logger.info("Performed target IAM Identity Center group assigment on account: %s, \
                                    Group: %s,  \
                                    Permission Set: %s", target_account_id, mapping['TargetGroupName'], each_perm_set_name)
                        logger.debug("Performed target IAM Identity Center group assigment on account: %s, \
                                    Group: %s,  \
                                    Permission Set: %s \
...
def get_all_permission_sets():
    ...
    try:
        ...
        for perm_set_arn in ic_permission_sets:
            ...
            logger.debug(f"Skipped Permission Set Name and ARN: {skipped_perm_set}")
...
def main(event=None):
    ...
    try:
        ...
        if delegated == "true":
            ...
            logger.debug("INFO: Admin delegated. Running in delegated admin account.")
        else:
...
if __name__ == "__main__":
    ...
    logger.info(f'Codebuild logs contain combined logs for the build project.\
                If you wish to view the logs for auto-assignment function, you can check out CloudWatch logs: "{assignment_automation_log_group}/[buildId]-{build_id}"\
